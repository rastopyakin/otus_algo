...
template <template <class> class Tree> double measure_search_random(int N) {
  ...
  // auto [tree, numbers] = make_random_tree<Tree>(N, d);

  std::pair<Tree<int>, std::vector<int>> p = make_random_tree<Tree>(N, d);
  auto &tree = p.first;
  auto &numbers = p.second;
  ...
  return avg_fn_result(1e-3, target_fn, N / 10);
}
...
template <template <class> class Tree> double measure_remove_ordered(int N) {
  Tree<int> tree;
  for (int i = 0; i < N; i++)
    tree.insert(i);
...
template <template <class> class Tree> double measure_remove_random(int N) {
  std::mt19937 gen;
  std::uniform_int_distribution<int> d{};

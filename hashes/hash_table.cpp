...
const std::size_t PrimeRehashPolicy::_primes[] = {
...
std::size_t PrimeRehashPolicy::_next_n_bkt_prime(std::size_t n_bkt) const {
  constexpr auto n_primes = sizeof(_primes) / sizeof(_primes[0]);
  constexpr auto last = _primes + n_primes - 1; // the last element should exist

  const auto next_bkt = std::lower_bound(_primes, last, n_bkt);

  if (next_bkt == last)
    // never rehash again
    _next_resize = std::size_t(-1);
  else
    _next_resize = std::floor(*next_bkt * _max_load_factor);

  return *next_bkt;
}

std::pair<bool, std::size_t> PrimeRehashPolicy::need_rehash(std::size_t n_bkt, std::size_t n_elem,
                                                            std::size_t n_ins) const {

  if (n_elem + n_ins > _next_resize) {
    float n_bkt_min = (n_elem + n_ins) / _max_load_factor;
    if (n_bkt_min > n_bkt)
      // here _next_resize is modified by _next_prime
      return {true, _next_n_bkt_prime(std::max<std::size_t>(std::floor(n_bkt_min) + 1, n_bkt * 2))};

    _next_resize = std::floor(n_bkt * _max_load_factor);
    return {false, 0};
  }
